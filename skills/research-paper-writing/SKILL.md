...
This skill runs on Windows. Toolchain (install once):

- **LaTeX** — MiKTeX ships `latexmk`, `pdflatex`, `biber`/`bibtex` and `chktex`:
  ```powershell
  winget install MiKTeX.MiKTeX
  initexmf --set-config-value="[MPM]AutoInstall=1"   # let it fetch missing packages on the fly
  latexmk -pdf main.tex                             # run from the paper directory; latexmk -C cleans up
  ```
- **Research Python** — use a dedicated venv (not the agent's own interpreter) with
  `semanticscholar arxiv habanero requests scipy numpy matplotlib SciencePlots pandas`:
  ```powershell
  uv venv "$env:USERPROFILE\paper-tools\venv"
  uv pip install --python "$env:USERPROFILE\paper-tools\venv\Scripts\python.exe" `
      semanticscholar arxiv habanero requests scipy numpy matplotlib SciencePlots pandas
  ```
  Call the pipeline scripts with that interpreter explicitly.
- **Differences from the examples in this SKILL.md** (they are written for linux/macOS):
  - There is no `python3` on Windows: use `python` or the venv path above.
  - No `nohup cmd &` for long experiments: use `Start-Process` (see Step 3.1) or a
    Hermes background terminal session with `notify`.
  - The example `.sh` helpers (`reproduce_table1.sh`, `download_data.sh`) are
    illustrative: on Windows write the equivalent in `.py` or `.ps1` — a `.sh` runs
    under whichever bash happens to be found, and a `C:\...` path can get mangled.
  - `templates/neurips2025/` ships a `Makefile`; on Windows skip it and run
    `latexmk -pdf main.tex` in that directory instead.
...
```powershell
# Windows: run in the background with Start-Process, logging to a file
$p = Start-Process -NoNewWindow -PassThru -RedirectStandardOutput logs\experiment_01.log `
     -FilePath "$env:USERPROFILE\paper-tools\venv\Scripts\python.exe" `
     -ArgumentList "run_experiment.py","--config","config.yaml"
$p.Id   # Record the PID; check later with: Get-Process -Id $p.Id
```
...
**Python deps** — the pipeline (citations, statistics, figures) needs these:
```powershell
# Windows (dedicated venv, see the Windows notes at the top of this skill)
"$env:USERPROFILE\paper-tools\venv\Scripts\python.exe" -c "import arxiv, semanticscholar, habanero, scipy, matplotlib, scienceplots; print('ok')"
```
```bash
# linux / macOS
pip install arxiv semanticscholar habanero requests scipy numpy matplotlib SciencePlots
```
